		 ...
		 void *compass_handle, void *pressure_handle)
{
...
#ifndef __KERNEL__
	/* The kernel module calls init and exit when a slave registers */
	for (ii = 0; ii < EXT_SLAVE_NUM_TYPES; ii++) {

		if (!mldl_cfg->slave[ii] || !mldl_cfg->slave[ii]->init)
			continue;

		result = mldl_cfg->slave[ii]->init(slave_handle[ii],
						mldl_cfg->slave[ii],
	...
	if (mldl_cfg->slave[EXT_SLAVE_TYPE_ACCEL] &&
		...
		result = mldl_cfg->slave[EXT_SLAVE_TYPE_ACCEL]->config(
			slave_handle[EXT_SLAVE_TYPE_ACCEL],
...
out_slave_failure:
	...
	for (ii = ii - 1; ii >= 0; ii--)
		if (mldl_cfg->slave[ii] && mldl_cfg->slave[ii]->exit)
			mldl_cfg->slave[ii]->exit(slave_handle[ii],
						mldl_cfg->slave[ii],
						mldl_cfg->pdata_slave[ii]);
	return result;
#endif
		  ...
		  void *pressure_handle)
{
	...
	for (ii = 0; ii < EXT_SLAVE_NUM_TYPES; ii++) {
		if (!mldl_cfg->slave[ii] || !mldl_cfg->slave[ii]->exit)
			continue;
		result = mldl_cfg->slave[ii]->exit(slave_handle[ii],
						mldl_cfg->slave[ii],
						mldl_cfg->pdata_slave[ii]);
