...
__open_again:
    ...
    if (pcm->fd < 0) {
		...
		printf("%s line=%d cannot open device=%s \n", __FUNCTION__, __LINE__, dname);
	...
	printf("%s line=%d pcm->fd=%d \n",__FUNCTION__, __LINE__, pcm->fd);
    ...
    if (ioctl(pcm->fd, SNDRV_PCM_IOCTL_HW_PARAMS, &params)) {
		...
		printf("%s line=%d cannot set hw params\n",  __FUNCTION__, __LINE__);
        goto fail;
    }
    param_dump(&params);
    ...
    if (ioctl(pcm->fd, SNDRV_PCM_IOCTL_SW_PARAMS, &sparams)) {
		...
		printf("%s line=%d cannot set sw params\n", __FUNCTION__, __LINE__);
        goto fail;
    }
    ...
    pcm->underruns = 0;
	printf("%s line=%d is done\n",  __FUNCTION__, __LINE__);
    return pcm;

fail:
	printf("%s line=%d is failed \n", __FUNCTION__, __LINE__);
    close(pcm->fd);
    pcm->fd = -1;
    return pcm;
}
