...
public class FmReceiveView extends FrameLayout implements View.OnClickListener,
    ...
    /* 
     * Signal strength boundaries for selecting GUI display.
     * Note that the lower the number, the better the signal.
     */
...
//	@Override
	public void onClick(View v) {
		...
		if (buttonId >= FmConstants.BUTTON_CH_1 && buttonId <= FmConstants.BUTTON_CH_10) {
			...
			mTouchEventhandler.handleButtonEvent(buttonId,
	 ...
	 * 
	 * @param strength SIGNAL_STRENGTH_NONE, SIGNAL_STRENGTH_LOW
	 ...
	 * 
	 * @param rdsState RDS_STATE_RDS_OFF, RDS_STATE_RDS_ON or
	 * RDS_STATE_RBDS_ON
	 */
	 ...
	 * 
	 * @param afState AF_STATE_OFF or AF_STATE_ON
	 */
	...
	/**
	 * Will update the display strings for the supported types of RDS
	 * data.
	 * 
	 * @param rdsDataType the type of RDS data received (RT, PTY, PTYN etc).
	 * @param rdsIndex the value of the RDS data if in integer form.
	 * @param rdsText the text of RDS data if available.
	 */
	public synchronized void setRdsText(int rdsDataType, int rdsIndex,
			String rdsText) {
	...
	public synchronized void setSeekStatus(boolean seekInProgress, boolean tuneInProgress) {
		findViewById(R.id.seek_indicator).setVisibility(seekInProgress || tuneInProgress ? View.VISIBLE : View.INVISIBLE);
