...
{
    BTIF_TRACE_DEBUG0("BTIF ENABLE RADIO");
...
bt_status_t btif_disable_bluetooth(void)
{
    ...
    else
    {
        ...
        BTIF_TRACE_DEBUG0("BTIF DISABLE BLUETOOTH MODULES");
...
{
    BTIF_TRACE_DEBUG2("btif_check_send_bt_off btif_core_state= (%d),ACL links = (%d)",
...
bt_status_t btif_disable_radio(void)
{
    ...
    if (0 == btif_core_radio_ref_count){
        ...
        {
            BTIF_TRACE_ERROR0("btif_disable_radio : not yet enabled");
            return BT_STATUS_NOT_READY;
        }

        BTIF_TRACE_DEBUG0("BTIF DISABLE RADIO");
		BTA_GpsDisable();
        ...
        if (status != BTA_SUCCESS)
        {
            BTIF_TRACE_ERROR1("disable radio failed (%d)", status);
