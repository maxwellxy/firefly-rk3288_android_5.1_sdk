...
class EntrypointsOrderTest : public CommonRuntimeTest {
 protected:
  ...
  void CheckQuickEntryPoints() {
    ...
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pCmplFloat, pCos, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pCos, pSin, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pSin, pAcos, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pAcos, pAsin, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pAsin, pAtan, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pAtan, pAtan2, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pAtan2, pCbrt, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pCbrt, pCeil, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pCeil, pCosh, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pCosh, pExp, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pExp, pExpm1, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pExpm1, pFloor, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pFloor, pHypot, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pHypot, pLog, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pLog, pLog10, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pLog10, pNextAfter, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pNextAfter, pRint, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pRint, pSinh, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pSinh, pTan, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pTan, pTanh, kPointerSize);
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pTanh, pFmod, kPointerSize);
