...
then
	...
	if [ $TARGET_PRODUCT == rk3188 ]
	then 
	mkbootimg --kernel $OUT/kernel --ramdisk $OUT/ramdisk.img --output $OUT/recovery.img && \
	cp -a $OUT/recovery.img $IMAGE_PATH/
	echo "done."
	else
	mkbootimg --kernel $OUT/kernel --ramdisk $OUT/ramdisk.img --second kernel/resource.img --output $OUT/recovery.img && \
	cp -a $OUT/recovery.img $IMAGE_PATH/
	echo "done."
	fi	
else
