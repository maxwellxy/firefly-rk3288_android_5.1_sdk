...
static
int (*const decoder_table[2])(struct mad_stream *, struct mad_frame *) = {
  mad_layer_I,
  mad_layer_II
};
...
void mad_frame_init(struct mad_frame *frame)
{
  ...
  /* by Vincent */
  frame->synth_out_samples_limit = 0;
...
int mad_frame_decode(struct mad_frame *frame, struct mad_stream *stream)
{
  ...
  /*
	  ...
	  mpeg 2.5  : 576
  */
  //if (frame->synth_out_samples_limit == 0)
  {
	  if (frame->header.layer == MAD_LAYER_I)
	  	frame->synth_out_samples_limit = 384; 	/* layer I */
	  else
	  	if (frame->header.layer == MAD_LAYER_III)
		{
			if (frame->header.flags & MAD_FLAG_LSF_EXT)	// mpeg 2.5
				frame->synth_out_samples_limit = 576;	/* layer III , MPEG 2.5 */
			else
				frame->synth_out_samples_limit = 1152;	/* layer III */
		}
		else
		{
			frame->synth_out_samples_limit = 1152;	/* layer III */
    ...
    if (frame->synth_out_samples_limit==0)
        frame->synth_out_samples_limit = 1152;
