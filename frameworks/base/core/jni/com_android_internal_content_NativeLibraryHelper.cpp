        ...
        jlong apkHandle, jobjectArray javaCpuAbisToSearch, jstring apkPkgName, jstring apkCodePath)
{
    ...
    do {
        ...
        ALOGV("IN HOUDINI findSupportedAbi %s", name.c_str());

        ABIList al(&supportedAbis);
        abiType = findSupportedAbi(env, apkHandle, javaCpuAbisToSearch);
        ALOGV("original findSupportedAbi return %d[%s]", abiType, al.getNameByIdx(abiType));
        ...
        ALOGV("new findSupportedAbi return %d[%s]", abiType, al.getNameByIdx(abiType));
    } while(false);
