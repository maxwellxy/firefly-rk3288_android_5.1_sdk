...
int main(int argc, char **argv) {
    ...
    // try interpreting the first arg as the socket name - if it fails go back to displayd

    if ((sock = socket_local_client(argv[1],
                                     ANDROID_SOCKET_NAMESPACE_RESERVED,
                                     SOCK_STREAM)) < 0) {
        if ((sock = socket_local_client("displayd",
                                         ANDROID_SOCKET_NAMESPACE_RESERVED,
                                         SOCK_STREAM)) < 0) {
    ...
    while(1) {
        ...
        if ((rc = select(sock +1, &read_fds, NULL, NULL, &to)) < 0) {
            ...
            if ((rc = read(sock, buffer, 4096)) <= 0) {
                if (rc == 0)
                    fprintf(stderr, "Lost connection to Displayd - did it crash?\n");
                else
